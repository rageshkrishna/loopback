...
AccessToken.prototype.getPrincipalType = function() {
  if(this.userId) {
    return ACL.USER;
  } else {
    return ACL.ROLE;
...
AccessToken.prototype.getPrincipalId = function() {
  switch(this.getPrincipalType()) {
    case ACL.USER:
      return this.userId;
    break;
    case ACL.ROLE:
...
AccessToken.prototype.canInvoke = function(remoteMethod, cb) {
  var principalType = this.getPrincipalType();
  var principalId = this.getPrincipalId();
  var Model = remoteMethod.ctor;
  var methodName = remoteMethod.name;

  ACL.checkPermission(principalType, principalId, Model.modelName, methodName, ACL.EXECUTE, cb);
}
